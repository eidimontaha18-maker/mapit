...
async function checkZonesTable() {
  try {
    ...
    // Try to insert a test zone with proper UUID
    console.log('\n🧪 Testing zone insertion with proper UUID...');
    ...
    try {
      const testInsert = await pool.query(`
        ...
        VALUES ($1, $2, $3, $4, $5, $6) 
        RETURNING *
      `, [testUuid, 18, 26, 'Test Zone', '#FF0000', JSON.stringify([[33.8547, 35.8623], [33.8548, 35.8624]])]);
      console.log('✅ Test insert successful:', testInsert.rows[0]);
      
      // Clean up test zone
      await pool.query('DELETE FROM zones WHERE id = $1', [testUuid]);

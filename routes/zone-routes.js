...
router.post('/', async (req, res) => {
  try {
    const { id, map_id, name, color, coordinates } = req.body;
    
    console.log('📥 Received zone creation request:', { id, map_id, name, color, coordinates: coordinates ? `${coordinates.length} points` : 'none' });
    
    if (!id || !map_id || !name || !color || !coordinates) {
      console.error('❌ Missing required fields');
      return res.status(400).json({ 
        success: false, 
        error: 'Missing required fields: id, map_id, name, color, coordinates' 
      });
    }
    
    // Get the customer_id directly from the map table
    const mapQuery = `
      SELECT customer_id 
      FROM map 
      WHERE map_id = $1 
      LIMIT 1
    `;
    const mapResult = await db.query(mapQuery, [map_id]);
    
    if (mapResult.rows.length === 0) {
      console.error('❌ Map not found:', map_id);
      return res.status(404).json({ 
        success: false, 
        error: 'Map not found or has no owner' 
      });
    }
    
    const customer_id = mapResult.rows[0].customer_id;
    console.log('✅ Found customer_id:', customer_id, 'for map_id:', map_id);
    
    // Check for duplicate zone ID
    const checkDuplicate = await db.query('SELECT id FROM zones WHERE id = $1', [id]);
    if (checkDuplicate.rows.length > 0) {
      console.log('⚠️ Zone already exists, updating instead');
      // Update existing zone
      const updateQuery = `
        UPDATE zones 
        SET name = $1, color = $2, coordinates = $3, map_id = $4, customer_id = $5, updated_at = CURRENT_TIMESTAMP
        WHERE id = $6
        RETURNING *
      `;
      const updateResult = await db.query(updateQuery, [name, color, JSON.stringify(coordinates), map_id, customer_id, id]);
      return res.status(200).json({ success: true, zone: updateResult.rows[0], updated: true });
    }
    ...
    const result = await db.query(query, [id, customer_id, map_id, name, color, JSON.stringify(coordinates)]);
    console.log('✅ Zone created successfully:', result.rows[0].id);

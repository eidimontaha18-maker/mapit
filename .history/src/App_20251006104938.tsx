...
const HomePage = ({ onLogout }: { onLogout: () => void }) => {
  ...
  const [highlightedCountry, setHighlightedCountry] = useState<string | undefined>();
  const [showAllCities, setShowAllCities] = useState(true);

  // On initial load, let's get all cities
  useEffect(() => {
    // This will be triggered from the CountrySidebar component
  }, []);

  const handleCountrySearch = (lat: number, lng: number, zoom: number, cities?: CityMarker[], countryName?: string) => {
    ...
    // Always set the highlighted country when provided
    if (countryName) {
      setHighlightedCountry(countryName);
    }
    
    // Keep showing all cities

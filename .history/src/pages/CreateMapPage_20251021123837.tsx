...
const CreateMapPage: React.FC = () => {
  ...
  React.useEffect(() => {
    if (!mapCode) {
      ...
      setMapCode(newCode);
      console.log('🔖 Generated map code:', newCode);
    }
  ...
  const handleSave = async () => {
    ...
    console.log('📦 Pending zones at save time:', pendingZones);
    console.log('📦 Number of pending zones:', pendingZones.length);
  ...
  const handleZoneCreated = useCallback((zone: Zone) => {
    ...
    setPendingZones(prev => {
      ...
      if (exists) {
        console.log('⚠️  Zone already exists in pending zones, skipping duplicate');
        return prev;
      }
      const newPendingZones = [...prev, zone];
      console.log(`✅ Zone added to pending zones. Total pending: ${newPendingZones.length}`);
  ...
  const savePendingZones = async (createdMapId: number, customerId: number): Promise<{success: boolean, count: number}> => {
    ...
    for (const zone of pendingZones) {
      try {
        ...
        if (result.success) {
          console.log(`✅ Zone "${zone.name}" saved successfully to database`);
          successCount++;

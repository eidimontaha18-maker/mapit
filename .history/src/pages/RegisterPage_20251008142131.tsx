...
// Enable client-side only mode if we're having server connectivity issues
const CLIENT_SIDE_ONLY = true; // Set to true if server isn't available

// Debug info for troubleshooting
console.log('RegisterPage loaded. Client-side only mode:', CLIENT_SIDE_ONLY);
if (!CLIENT_SIDE_ONLY) {
  console.log('Will try these API URLs:', API_PORTS.map(port => `${API_BASE}${port}`));
}
...
const RegisterPage: React.FC = () => {
  ...
  // Save customer to backend API or simulate saving in client-side only mode
  const saveCustomer = async (e?: React.FormEvent) => {
    ...
    if (CLIENT_SIDE_ONLY) {
      console.log('Running in client-side only mode (no server connection)');
      console.log('Form data that would be sent:', form);
      
      // Simulate network delay
      await new Promise(resolve => setTimeout(resolve, 1000));
      
      // In client-side mode, just show success and reset the form
      setStatus('Registration successful! (Client-side only mode)');
      setForm({ first_name: '', last_name: '', email: '', password: '' });
      return;
    }
    
    // If not in client-side only mode, try each API port until one works

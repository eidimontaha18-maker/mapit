...
const CreateMapPage: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      try {
        const dbCheckResponse = await fetch('http://127.0.0.1:3101/api/db/status', {
          method: 'GET',
          signal: AbortSignal.timeout(2000) // Short timeout to check connectivity
        });
        
        if (!dbCheckResponse.ok) {
          console.warn('Database connection check failed, but will try to save anyway');
        } else {
          console.log('Database connection confirmed');
        }
      } catch (dbCheckError) {
        console.warn('Could not verify database connection:', dbCheckError);
      }

  ...
  useEffect(() => {
    fetch('/api/packages')
      .then(async (res) => {
        ...
        if (!res.ok) {
          console.error('Failed to fetch packages:', text);
          return null;
        }
      ...
      .then((data) => {
        if (data?.success && data?.packages) {
          ...
          console.log('Available packages:', data.packages);
        }
      })
      .catch(error => {
                    ...
                    >
                      ...
                      <td style={{ padding: '16px', borderBottom: '1px solid #eaedf3' }}>
                        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                          <button 
                            onClick={() => {
                              const mapUrl = `${window.location.origin}/view-map/${map.map_id}?type=road`;
        ...
        >
          ...
          >
            ...
            <div style={{
              ...
              {availablePackages
                .filter(pkg => customerPackage && pkg.priority > customerPackage.priority)
                .map((pkg) => {
                  ...
                  return (
                    <div
                      key={pkg.package_id}
                      style={{
                        border: isPopular ? '2px solid #667eea' : '2px solid #e2e8f0',
                        borderRadius: '12px',
                        padding: '16px',
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div>
                          ...
                          <div style={{ fontSize: '14px', color: '#718096' }}>
                            Up to {pkg.allowed_maps} {pkg.allowed_maps === 1 ? 'map' : 'maps'}
                          </div>
                        </div>

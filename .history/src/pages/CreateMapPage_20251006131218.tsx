...
const CreateMapPage: React.FC = () => {
  ...
  const handleSearch = (lat: number, lng: number, zoom: number, cities?: CityMarker[], countryName?: string) => {
    ...
    let adjustedZoom;
    if (countryName) {
      // Define specific countries with custom zoom levels
      ...
      // Get specific zoom level if defined, otherwise use default
      adjustedZoom = countryZoomLevels[countryName] || 6;
      
      console.log('Country detected:', countryName, 'zoom set to:', adjustedZoom);
    } else {

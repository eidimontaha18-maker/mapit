      ...
      const fetchMapData = async () => {
        try {
          ...
          if (data.success && data.record) {
            ...
            if (mapData.map_codes && Array.isArray(mapData.map_codes) && mapData.map_codes.length > 0) {
              setMapCode(mapData.map_codes[0]);
            }
            
            // Load zones for this map
            try {
              const zonesResponse = await fetch(`/api/db/tables/zones?map_id=${id}`);
              const zonesData = await zonesResponse.json();
              
              if (zonesData.success && Array.isArray(zonesData.records)) {
                console.log('Loaded zones:', zonesData.records);

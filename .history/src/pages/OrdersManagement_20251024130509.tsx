  ...
  return (
    <div className="orders-management">
            ...
            {filteredOrders.length === 0 ? (
              ...
              filteredOrders.map((order) => (
                <tr key={order.order_id}>
                  ...
                  <td className="total-amount">${parseFloat(order.total).toFixed(2)}</td>
                  <td className="maps-count">{order.allowed_maps} maps</td>
                  <td className="order-datetime">
                    <div>
                      <strong>{new Date(order.order_date).toLocaleDateString('en-US', {
                        month: 'short',
                        day: 'numeric',
                        year: 'numeric'
                      })}</strong>
                      <small>{new Date(order.order_date).toLocaleTimeString('en-US', {
                        hour: '2-digit',
                        minute: '2-digit',
                        hour12: true
                      })}</small>
      ...
      <div className="orders-summary">
        ...
        <div className="summary-card">
          <h3>Completed</h3>
          <p className="summary-value completed-count">{orders.filter(o => o.status === 'completed').length}</p>
        </div>
        <div className="summary-card">
          <h3>Pending</h3>
          <p className="summary-value pending-count">{orders.filter(o => o.status === 'pending').length}</p>
        </div>
        <div className="summary-card">
          <h3>Total Revenue</h3>
          <p className="summary-value revenue-value">
            ${orders.reduce((sum, order) => sum + parseFloat(order.total || order.price || '0'), 0).toFixed(2)}
          </p>
        </div>
      </div>
    </div>

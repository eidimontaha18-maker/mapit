...
import AppN  // Enhanced search handler function for the sidebar with direct map controls
  const handleSearch = (lat: number, lng: number, zoom: number, cities?: CityMarker[], countryName?: string) => {
    ...
    // Force much higher zoom levels based on what we're looking at
    let adjustedZoom;
    if (countryName) {
      // Countries need different zoom levels based on size
      const largeCountries = ['Russia', 'Canada', 'United States', 'China', 'Brazil', 'Australia'];
      const mediumCountries = ['India', 'Argentina', 'Kazakhstan', 'Algeria', 'Mexico'];
      
      if (largeCountries.includes(countryName)) {
        adjustedZoom = 4; // Large countries need less zoom
      } else if (mediumCountries.includes(countryName)) {
        adjustedZoom = 5; // Medium countries
      } else {
        adjustedZoom = 6; // Smaller countries need more zoom
      }
    } else {
      // Cities or default locations
      adjustedZoom = 11;
    }
    
    // Make sure zoom is at least as high as requested, but not less than our minimum
    adjustedZoom = Math.max(zoom, adjustedZoom);
    
    console.log('Using adjusted zoom:', adjustedZoom);

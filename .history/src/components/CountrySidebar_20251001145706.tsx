...
const CountrySidebar: React.FC<CountrySidebarProps> = ({ onSearch }) => {
  ...
  // Only English supported, remove language state
  ...
  function getCitiesForCountry(country: string) {
    return Object.keys(allCities).filter(city => {
      const cityObj = (allCities as Record<string, any>)[city];
  ...
  const handleSearch = () => {
    ...
    if (cityKey) {
      setError('');
  const { lat, lng } = (allCities as Record<string, any>)[cityKey];
      onSearch(lat, lng, 11);
      return;
    }
    if (countryKey) {
      setError('');
  const { lat, lng } = (allCountries as Record<string, any>)[countryKey];
      onSearch(lat, lng, 7);
      return;
    }

...
const CountrySidebar: React.FC<CountrySidebarProps> = ({ onSearch }) => {
  ...
  const handleCountryInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.length > 0) {
      ...
      if (exactCountryMatch) {
        ...
        onSearch(lat, lng, 10, [], exactCountryMatch);
  ...
  const handleSearch = () => {
    ...
    if (cityKey) {
      ...
      if (countryOfCity) {
        ...
        // Just show the single city if we can't find its country with much closer zoom
        onSearch(cityData.lat, cityData.lng, 15);
      }
  ...
  // Enhanced version that prioritizes prefix matches first, then fuzzy matches
  function getBestMatch(input: string, options: string[]): string | undefined {
    input = input.toLowerCase();
    
    // First try exact prefix match for quick feedback while typing
      ...
      {!showMapForm && (
        ...
        <div className="search-row">
          <input
            ...
            autoComplete="off"
          />
          <button className="sidebar-btn search-button" onClick={handleSearch} title="Search for exact match">

...
// Export Zone interface so it can be used by the API service
export interface Zone {
  id: string;
  name: string;
  color: string;
  // Store the polygon coordinates to recreate the polygon
...
interface WorldMapProps {
  ...
  userId?: number; // User ID for saving zones
}
...
const WorldMap: React.FC<WorldMapProps> = ({
  ...
  useEffect(() => {
    const loadZones = async () => {
      try {
        const { default: zoneApi } = await import('../services/zoneApi');
        const userZones = await zoneApi.getZones();
        
        if (userZones && userZones.length > 0) {
          setZones(userZones);
          console.log(`Loaded ${userZones.length} zones from database`);
        }
      } catch (err) {
  ...
  const handleSaveAllZones = useCallback(() => {
    try {
      import('../services/zoneApi').then(({ default: zoneApi }) => {
        zoneApi.saveZones(zones).then(() => {
      ...
      console.error('Error importing zoneApi:', err);
    }
  }, [zones]);
  ...
  const handleZoneCreated = useCallback((zoneData: Zone) => {
    ...
    try {
      import('../services/zoneApi').then(({ default: zoneApi }) => {
        zoneApi.createZone(zoneData).catch(err => {
  ...
  const handleDeleteZone = useCallback((id: string) => {
    ...
    try {
      import('../services/zoneApi').then(({ default: zoneApi }) => {
        zoneApi.deleteZone(id).catch(err => {

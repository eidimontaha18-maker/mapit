...
interface CityMarker {
  ...
  isCapital?: boolean;
        ...
        // Only update map view once per prop change
        useEffect(() => {
          if (lat !== null && lng !== null && zoom !== null && !isNaN(lat) && !isNaN(lng) && !isNaN(zoom)) {
            map.setView([lat, lng], zoom, { animate: false });
          }
 ...
 */
// React component for displaying world map with search functionality
const WorldMap: React.FC<WorldMapProps> = ({ 
  ...
  // List of countries that need special handling for reliable focus using useMemo to avoid re-renders
  const CRUCIAL_COUNTRIES = React.useMemo(() => 
    ['Lebanon', 'Syria', 'Jordan', 'Israel', 'Palestine', 'Kuwait', 'Qatar', 'Bahrain']
  , []);
    ...
    if (highlightedCountry && highlightedCountry !== lastCountryRef.current) {
      ...
      if (countryCoords) {
        ...
        if (mapRef.current) {
          ...
          // Is this a crucial country that needs special handling?
          const isCrucialCountry = CRUCIAL_COUNTRIES.includes(highlightedCountry);
          
          // Apply different strategies based on country importance
          if (isCrucialCountry) {
            console.log(`${highlightedCountry} is a CRUCIAL country - applying special handling`);
            // For crucial countries like Lebanon, use a more aggressive approach
            // First set view immediately
            mapRef.current.setView([countryLat, countryLng], countryZoom, { animate: false });
            
            // Then create a series of delayed updates at different intervals
  ...
  return (
    <div style={{ 
      ...
      <MapContainer 
        key={`map-${highlightedCountry || 'world'}-${lat}-${lng}-${zoom}-${new Date().getTime()}`} // Force complete re-creation with all parameters
        ...
        zoomControl={true}
        // Using whenCreated prop which is deprecated but still works

...
const WorldMap: React.FC<WorldMapProps> = ({
  ...
  const [drawingZoneDetails, setDrawingZoneDetails] = useState<{ name: string; color: string }>({ name: "", color: "#3388ff" });
  ...
  const handleStartDrawing = (name: string, color: string) => {
    ...
    if (mapRef.current && featureGroupRef.current) {
      // Create a new polygon draw handler
      const drawControl = new L.Draw.Polygon(mapRef.current);
      
      // Set up the event handlers for drawing
      mapRef.current.once('draw:created', (e: L.DrawEvents.Created) => {

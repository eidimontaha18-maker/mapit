...
const CountrySidebar: React.FC<CountrySidebarProps> = ({ onSearch }) => {
  ...
  const [maps, setMaps] = useState<Array<{ id: number; title: string; description: string }>>([]);
  ...
  return (
    <div className={`country-sidebar ${isSidebarOpen ? 'open' : 'closed'}`}>
      ...
      >
        <span className={`chevron-icon ${isSidebarOpen ? '' : 'right'}`}></span>
      </button>
      <div className="sidebar-header">
        <h2>Location Search</h2>
        <p>Start typing a country or city name - map will zoom automatically!</p>
      </div>
      ...
      {/* Plus button for new map creation, only if logged in */}
      {isLoggedIn && (
        <button
          className="sidebar-btn"
          style={{ margin: '10px 0', fontSize: '1.5em', width: '40px', height: '40px', borderRadius: '50%' }}
      ...
      {showMapForm && (
        <div className="sidebar-map-form" style={{ padding: '10px', background: '#f9f9f9', borderRadius: '8px', marginBottom: '10px' }}>
          ...
          >Save</button>
          <button className="sidebar-btn" style={{ marginLeft: '8px' }} onClick={() => setShowMapForm(false)}>Cancel</button>
      ...
      {maps.length > 0 && (
        <div className="sidebar-maps-list" style={{ marginBottom: '10px' }}>
          <h4>My Maps</h4>
          <ul style={{ listStyle: 'none', padding: 0 }}>
            {maps.map(map => (
              <li key={map.id} style={{ marginBottom: '6px', padding: '4px', background: '#eee', borderRadius: '4px' }}>
                <strong>{map.title}</strong><br />
                <span style={{ fontSize: '0.9em', color: '#555' }}>{map.description}</span>
              </li>
            ))}
          </ul>
        </div>

...
interface CountrySidebarProps {
  onSearch: (lat: number, lng: number, zoom: number, cities?: CityMarker[]) => void;
}
  ...
  const handleCountrySuggestionClick = (suggestion: string) => {
    ...
    if (suggestion in allCities) {
      ...
      if (countryOfCity) {
        ...
        onSearch(cityData.lat, cityData.lng, 11, cityMarkers);
      } else {
    ...
    // Check if it's a country
      ...
      // Create markers for all cities in this country
      const cityMarkers = createCityMarkers(suggestion);
      
      // Zoom to country and show all city markers
      const { lat, lng } = (allCountries as Record<string, { lat: number; lng: number }>)[suggestion];
      onSearch(lat, lng, 7, cityMarkers);
    }
  };

  const handleCityChange = (e: React.ChangeEvent<HTMLSelectElement> | { target: { value: string } }) => {
    ...
    if (cityName) {
      ...
      if (cityData) {
        // Send just the single city to zoom to it
        onSearch(cityData.lat, cityData.lng, 11);
      }
    }
  };
  ...
  const handleSearch = () => {
    ...
    if (cityKey) {
      ...
      if (countryOfCity) {
        ...
        // Move map to city position and show all city markers
        onSearch(cityData.lat, cityData.lng, 11, cityMarkers);
      } else {
        // Just show the single city if we can't find its country
        onSearch(cityData.lat, cityData.lng, 11);
      }
    ...
    if (countryKey) {
      ...
      // Create markers for all cities in this country
      const cityMarkers = createCityMarkers(countryKey);
      
      // Move map to country position and show all city markers
      onSearch(lat, lng, 7, cityMarkers);
      return;
    }
              ...
              {getCitiesForCountry(selectedCountry).map((city: string) => (
                <div 
                  ...
                  onClick={() => {
                    ...
                    const cityData = (allCities as Record<string, { lat: number; lng: number; ar?: string }>)[city];
                    if (cityData) {
                      // Get all city markers for the current country
                      const cityMarkers = createCityMarkers(selectedCountry);
                      // Zoom to the selected city but show all markers
                      onSearch(cityData.lat, cityData.lng, 11, cityMarkers);
                    }
                  }}
                >

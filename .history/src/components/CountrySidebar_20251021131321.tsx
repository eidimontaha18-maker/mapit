...
const CountrySidebar: React.FC<CountrySidebarProps> = ({ 
  ...
  const visibleCities = React.useMemo(() => {
    ...
    console.log('🗺️ Filtering cities within map bounds:', mapBounds);
    ...
    const filtered = countryCities.filter(city => {
      ...
      if (isWithinBounds) {
        console.log(`✅ City in viewport: ${city.name} (${city.lat}, ${city.lng})`);
      }
      
      return isWithinBounds;
    });

    console.log(`📍 Showing ${filtered.length} of ${countryCities.length} cities in current viewport`);
    return filtered;
  ...
  const fetchCitiesViaOverpass = React.useCallback(async (countryName: string): Promise<Array<{name: string; lat: number; lng: number}>> => {
    try {
      ...
      const countryISOCodes: Record<string, string> = {
        ...
        'Uruguay': 'UY', 'Costa Rica': 'CR', 'Panama': 'PA', 'Cuba': 'CU',
        ...
        // Middle East
        'Lebanon': 'LB', 'Syria': 'SY', 'Egypt': 'EG', 'Turkey': 'TR',
        'United Arab Emirates': 'AE', 'Saudi Arabia': 'SA', 'Israel': 'IL',
        'Palestine': 'PS', 'Jordan': 'JO', 'Iraq': 'IQ', 'Kuwait': 'KW',
        'Qatar': 'QA', 'Bahrain': 'BH', 'Oman': 'OM', 'Yemen': 'YE', 'Iran': 'IR',
        
        // Asia
        'China': 'CN', 'Japan': 'JP', 'India': 'IN', 'Indonesia': 'ID',
        'Thailand': 'TH', 'Vietnam': 'VN', 'Philippines': 'PH', 'Malaysia': 'MY',
        'Singapore': 'SG', 'Pakistan': 'PK', 'Bangladesh': 'BD', 'South Korea': 'KR',
        'North Korea': 'KP', 'Myanmar': 'MM', 'Cambodia': 'KH', 'Laos': 'LA',
        'Nepal': 'NP', 'Sri Lanka': 'LK', 'Afghanistan': 'AF', 'Mongolia': 'MN',
        
        // Africa
        'South Africa': 'ZA', 'Nigeria': 'NG', 'Kenya': 'KE', 'Morocco': 'MA',
        'Algeria': 'DZ', 'Ethiopia': 'ET', 'Ghana': 'GH', 'Tanzania': 'TZ',
        'Uganda': 'UG', 'Tunisia': 'TN', 'Libya': 'LY', 'Sudan': 'SD',
      ...
      // Query Overpass API for cities and towns with English names
      const query = `
        [out:json][timeout:25];
      ...
      data.elements.forEach((element: any) => {
        if (element.lat && element.lon) {
          // Priority order for name selection: English > International > Default
          const cityName = 
            element.tags?.['name:en'] ||      // English name
            element.tags?.['int_name'] ||     // International name
            element.tags?.['name:latin'] ||   // Latin script name
            element.tags?.name;                // Default name (may be local script)
      ...
      return cities.sort((a, b) => a.name.localeCompare(b.name)).slice(0, 100);
    } catch (error) {
  ...
  const getCitiesForCountry = React.useCallback(async (countryName: string): Promise<Array<{name: string; lat: number; lng: number}>> => {
    console.log(`🔍 Fetching cities specifically for country: ${countryName}`);
    
    try {
      setIsLoadingCities(true);
      
      // Get country ISO code for better API queries
      const countryISOCodes: Record<string, string> = {
        'United States': 'US', 'United Kingdom': 'GB', 'France': 'FR', 'Germany': 'DE',
        'Italy': 'IT', 'Spain': 'ES', 'Canada': 'CA', 'Brazil': 'BR', 'India': 'IN',
        'China': 'CN', 'Japan': 'JP', 'Australia': 'AU', 'Lebanon': 'LB', 'Syria': 'SY',
        'Egypt': 'EG', 'United Arab Emirates': 'AE', 'Saudi Arabia': 'SA', 'Turkey': 'TR',
        'Russia': 'RU', 'Mexico': 'MX', 'Argentina': 'AR', 'South Africa': 'ZA',
        'Nigeria': 'NG', 'Kenya': 'KE', 'Morocco': 'MA', 'Algeria': 'DZ', 'Indonesia': 'ID',
        'Thailand': 'TH', 'Vietnam': 'VN', 'Philippines': 'PH', 'Malaysia': 'MY',
        'Singapore': 'SG', 'Pakistan': 'PK', 'Bangladesh': 'BD', 'Netherlands': 'NL',
        'Belgium': 'BE', 'Sweden': 'SE', 'Norway': 'NO', 'Denmark': 'DK', 'Finland': 'FI',
        'Poland': 'PL', 'Greece': 'GR', 'Portugal': 'PT', 'Switzerland': 'CH',
        'Austria': 'AT', 'Ireland': 'IE', 'New Zealand': 'NZ', 'Chile': 'CL',
        'Colombia': 'CO', 'Peru': 'PE', 'Venezuela': 'VE', 'Israel': 'IL', 'Palestine': 'PS',
        'Jordan': 'JO', 'Iraq': 'IQ', 'Kuwait': 'KW', 'Qatar': 'QA', 'Bahrain': 'BH',
        'Oman': 'OM', 'Yemen': 'YE', 'Iran': 'IR'
      };
      
      const isoCode = countryISOCodes[countryName];
      console.log(`📍 Country ISO code: ${isoCode || 'Not found, using country name'}`);
      
      // Enhanced queries with explicit country filtering and English names
      const queries = [
        // Query 1: Cities with ISO code (most accurate)
        fetch(
        ...
        // Query 3: Administrative places (cities, towns, villages)
        fetch(
          `https://nominatim.openstreetmap.org/search?` +
          `countrycodes=${encodeURIComponent(isoCode || '')}` +
          `&format=json` +
          `&limit=150` +
          `&addressdetails=1` +
          `&namedetails=1` +
      ...
      console.log(`📦 Raw API data for ${countryName}:`, validData.map(d => `${d.length} items`).join(', '));
      
      // Combine and parse all responses
      const fetchedCities: Array<{name: string; lat: number; lng: number}> = [];
      const seenCities = new Set<string>();
      
      validData.forEach((data, queryIndex) => {
        console.log(`📊 Processing query ${queryIndex + 1}: ${data.length} items from API...`);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        data.forEach((item: any) => {
          // STRICT BOUNDARY CHECK: Verify city is actually in the requested country
          const itemCountry = item.address?.country;
          const itemCountryCode = item.address?.country_code?.toUpperCase();
          const requestedCountryCode = isoCode?.toUpperCase();
          
          // Multi-level validation for country match
          const isCountryMatch = 
            // Method 1: Exact country name match
            (itemCountry && itemCountry.toLowerCase() === countryName.toLowerCase()) ||
            // Method 2: ISO country code match (most reliable)
            (requestedCountryCode && itemCountryCode === requestedCountryCode) ||
            // Method 3: Display name contains country
            (item.display_name?.toLowerCase().includes(countryName.toLowerCase()));
          
          if (!isCountryMatch) {
            console.log(`❌ Rejected: ${item.display_name || item.name} - Country mismatch (expected: ${countryName}, got: ${itemCountry || 'unknown'})`);
            return; // Skip this item - it's not in the requested country
          }
          
          // Extract city name with priority order
          // First, try to get English name from namedetails
          let cityName = 
            item.namedetails?.['name:en'] ||  // English name
            item.address?.city || 
            item.address?.town || 
          ...
          // Fallback to item name for specific place types
          if (!cityName && ['city', 'town', 'village', 'administrative'].includes(item.type)) {
            cityName = item.name;
          }
          ...
          if (!cityName || 
            ...
            return; // Skip invalid or duplicate cities
          }
          ...
          // Create unique key using name and coordinates to handle cities with same name
          ...
          console.log(`✅ Added: ${cityName} (${lat.toFixed(4)}, ${lng.toFixed(4)}) in ${countryName}`);
        });
      });
      
      setIsLoadingCities(false);
      
      console.log(`📊 API RESULTS SUMMARY for ${countryName}:`);
      console.log(`   - Total unique cities found: ${fetchedCities.length}`);
      console.log(`   - Using country code: ${isoCode || 'N/A (using name)'}`);
      console.log(`   - First 10 cities:`, fetchedCities.slice(0, 10).map(c => c.name).join(', '));
      
      // If we found cities, return them sorted
      if (fetchedCities.length > 0) {
        const sortedCities = fetchedCities
          .sort((a, b) => a.name.localeCompare(b.name))
          .slice(0, 150); // Return up to 150 cities
        
        console.log(`✅ SUCCESS: Returning ${sortedCities.length} cities from ${countryName}`);
        return sortedCities;
      }
      
      // If no cities found via Nominatim, try Overpass API as fallback
      console.log(`⚠️ No cities found via Nominatim for ${countryName}, trying Overpass API...`);
      const overpassCities = await fetchCitiesViaOverpass(countryName);
      
      if (overpassCities.length > 0) {
        console.log(`✅ FALLBACK SUCCESS: Overpass API returned ${overpassCities.length} cities for ${countryName}`);
        return overpassCities;
      }
      
      console.log(`❌ FAILED: No cities found for ${countryName} from any API source`);
      return [];
      ...
      try {
        console.log(`🔄 Attempting Overpass API as emergency fallback...`);
        const overpassCities = await fetchCitiesViaOverpass(countryName);
        if (overpassCities.length > 0) {
          console.log(`✅ Emergency fallback successful: ${overpassCities.length} cities`);
          return overpassCities;
        }
      } catch (fallbackError) {
    ...
    const loadCitiesForCountry = async () => {
      if (selectedCountry) {
        ...
        if (coordinates) {
          ...
          // Multiply zoom by 7 for automatic zoom in
          const enhancedZoom = zoom * 7;
          onSearch(lat, lng, enhancedZoom, selectedCountry);
            ...
            onClick={() => {
              ...
              if (searchValue) {
                ...
                if (countryMapping[lowerSearchValue]) {
                  ...
                  // Multiply zoom by 7 for automatic zoom in
                  const enhancedZoom = coords.zoom * 7;
                  
                  // First zoom attempt
                  onSearch(coords.lat, coords.lng, enhancedZoom, countryName);
        ...
        {selectedCountry && (
          <div style={{
            ...
            <div style={{
              ...
              Cities in {selectedCountry} {visibleCities.length > 0 && `(${visibleCities.length}${mapBounds ? ' in view' : ''})`}
            </div>
            ...
            {/* City search input */}
            {!isLoadingCities && visibleCities.length > 3 && (
            ...
            <div style={{
              ...
              {/* No results message */}
              {visibleCities.filter(city => city.name.toLowerCase().includes(citySearchTerm.toLowerCase())).length === 0 && (
                <div style={{
                  padding: '20px',
                  textAlign: 'center',
                  color: '#64748b',
                  ...
                  {citySearchTerm ? `No cities found matching "${citySearchTerm}"` : 
                   mapBounds ? 'No cities in current viewport. Zoom out to see more.' : 
                   'No cities found'}
                </div>
              )}
            </div>

...
const CountrySidebar: React.FC<CountrySidebarProps> = ({ onSearch }) => {
  ...
  const handleSearch = () => {
    ...
    if (countryKey) {
      ...
      // Focus intently on countries, especially smaller ones like Lebanon
      console.log(`Zooming to ${countryKey} with coordinates:`, lat, lng);
      
      // Force update the map position with both initial call and a delayed call
      onSearch(lat, lng, zoomLevel, [], countryKey);
      
      // Follow up with a delayed call to ensure the zoom happens
      setTimeout(() => {
        console.log(`Delayed zoom to ${countryKey}`);
        onSearch(lat, lng, zoomLevel, [], countryKey);
      }, 300);
      return;
    }

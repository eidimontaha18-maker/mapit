...
const CountrySidebar: React.FC<CountrySidebarProps> = ({ onSearch }) => {
  ...
  const handleSearch = () => {
    ...
    if (countryKey) {
      ...
      console.log('Country zoom level:', zoomLevel);
      
      // Focus aggressively on countries with multiple forced updates
      console.log(`Zooming to ${countryKey} with coordinates:`, lat, lng, 'at zoom level:', zoomLevel);
      
      // First update - initial positioning
      onSearch(lat, lng, zoomLevel, [], countryKey);
      
      // Force a series of updates to ensure the map responds
      const forceUpdate = (delay: number) => {
        setTimeout(() => {
          console.log(`Forced update for ${countryKey} at ${delay}ms`);  
          // Tiny offset to force re-render
          const offset = delay / 10000;
          onSearch(lat + offset, lng + offset, zoomLevel, [], countryKey);
        }, delay);
      };
      
      // Multiple updates with increasing delays
      forceUpdate(200);
      forceUpdate(500);
      forceUpdate(1000);
    ...
    if (cityKey) {
      ...
      if (countryOfCity) {
        ...
        onSearch(cityData.lat, cityData.lng, 12, cityMarkers, countryOfCity);
      } else {
          ...
          <button 
            ...
            onClick={() => {
              ...
              if (searchValue) {
                ...
                // Check for Lebanon specifically
                const isLebanon = searchValue.toLowerCase() === 'lebanon';
                if (isLebanon) {
                  console.log('🇱🇧 Lebanon detected - using direct coordinates');
                  // Use hardcoded coordinates for Lebanon
                  const lebanon = {
                    lat: 33.8547,
                    lng: 35.8623,
                    zoom: 8
                  };
                  onSearch(lebanon.lat, lebanon.lng, lebanon.zoom, [], 'Lebanon');
                  
                  // Follow up with multiple attempts

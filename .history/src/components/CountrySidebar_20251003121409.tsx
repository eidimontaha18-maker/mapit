...
const CountrySidebar: React.FC<CountrySidebarProps> = ({ onSearch }) => {
  ...
  const handleCountrySuggestionClick = (country: string) => {
    ...
    setLocation(country);
    setCountrySuggestions([]);
    // Auto-search when clicking on a country suggestion
    const { lat, lng } = (allCountries as Record<string, { lat: number; lng: number }>)[country];
    onSearch(lat, lng, 7);
  };
  ...
  const handleSearch = () => {
    ...
    if (countryKey) {
      ...
      // When a country is found, update the selected country to populate cities

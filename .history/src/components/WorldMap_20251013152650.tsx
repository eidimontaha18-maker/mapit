...
const WorldMap: React.FC<WorldMapProps> = ({
    ...
    const loadMapInfo = async () => {
      ...
      try {
        ...
        if (data.success && data.record) {
          const mapCode = data.record.map_codes && data.record.map_codes.length > 0 
            ? data.record.map_codes[0] 
            : `MAP-${mapId}`;
  ...
  const handleZoneCreated = useCallback(async (zoneData: Zone) => {
    console.log('💾 Zone created, saving to database...', zoneData);
    ...
    try {
      ...
      if (mapId) {
        ...
        if (result.success) {
          ...
          setTimeout(() => {
            alert(`Zone "${zoneData.name}" saved successfully!`);
          }, 100);
        } else {

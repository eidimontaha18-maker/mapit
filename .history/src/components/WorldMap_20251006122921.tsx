...
function MapUpdater({ lat, lng, zoom }: { lat: number, lng: number, zoom: number }) {
  ...
  useEffect(() => {
    ...
    if (lat !== null && lng !== null && zoom !== null) {
      ...
      // Show a temporary message on the map for debugging
      const popup = L.popup({ autoClose: true, closeOnClick: false, className: 'map-update-popup' })
        .setLatLng([lat, lng])
        .setContent(`<div>Zooming to: ${lat.toFixed(2)}, ${lng.toFixed(2)} (zoom: ${zoom})</div>`)
        .openOn(map);
...
const WorldMap: React.FC<WorldMapProps> = ({ 
  ...
  // Enhanced map with location marker and city markers display
  return (
    <div style={{ 
      position: 'fixed', 
      top: NAVBAR_HEIGHT, 

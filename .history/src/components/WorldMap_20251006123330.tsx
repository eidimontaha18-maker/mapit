...
function MapUpdater({ lat, lng, zoom }: { lat: number, lng: number, zoom: number }) {
  ...
  // Force update the map view immediately when props change
  useEffect(() => {
    ...
    if (lat !== null && lng !== null && zoom !== null) {
      ...
      // Force the map view to immediately update to the new position
      ...
      // Add a marker at the destination for visibility
...
function LocationMarker({ lat, lng, label }: { lat: number, lng: number, label: string }) {
  return (
    ...
    >
      <Popup autoClose={false} closeOnClick={false} autoPan={true} closeButton={true} open={true}>
        <strong>{label}</strong>
        <br/>
  ...
  return (
    <div style={{ 
      ...
      >
        ...
        <div className="map-debug-overlay" style={{
          position: 'absolute',
          bottom: '10px',
          left: '10px',
          padding: '5px',
          ...
          Current view: {lat.toFixed(2)}, {lng.toFixed(2)}, zoom: {zoom}
          {highlightedCountry && <div>Country: {highlightedCountry}</div>}
        </div>
      </MapContainer>
    </div>
  );

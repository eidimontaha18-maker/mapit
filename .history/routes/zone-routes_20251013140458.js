...
router.post('/', async (req, res) => {
  try {
    ...
    // Get the customer_id from the map ownership
    const mapOwnerQuery = `
      ...
      LIMIT 1
    `;
    const mapOwnerResult = await db.query(mapOwnerQuery, [map_id]);
    
    if (mapOwnerResult.rows.length === 0) {
    ...
    const customer_id = mapOwnerResult.rows[0].customer_id;

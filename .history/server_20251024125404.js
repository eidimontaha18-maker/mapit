...
const HOST = process.env.SERVER_HOST || '127.0.0.1'; // Server listens on localhost, not DB host
...
app.get('/api/admin/orders', async (req, res) => {
  ...
  try {
    ...
    console.log('[GET /api/admin/orders] Found orders:', result.rows.length);
    res.json({ success: true, orders: result.rows });
  } catch (err) {

...
async function fixDatabaseStructure() {
  try {
    ...
    if (mapCodesCheck.rows.length > 0 && mapCodesCheck.rows[0].data_type === 'ARRAY') {
      ...
      // First, update existing records to use first element of array if exists
      await pool.query(`
        UPDATE map 
        SET map_codes = CASE 
      ...
      await pool.query('ALTER TABLE map DROP COLUMN IF EXISTS map_codes');
      await pool.query('ALTER TABLE map ADD COLUMN map_code VARCHAR(50) UNIQUE');

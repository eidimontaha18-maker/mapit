...
(async () => {
  ...
  const q = `SELECT cu.customer_id, cu.email, m.map_id, m.title as name, m.description, COALESCE(z.zone_count,0) as zone_count, m.created_at, m.active, m.country, m.map_code
FROM customer cu
